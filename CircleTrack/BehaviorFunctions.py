...
from CaImaging.util import find_closest, ScrollPlot, disp_frame, \
    ...
    contiguous_regions, sync_cameras_v4
...
def sync_Arduino_outputs(session_folder, behav_cam=0,
    ...
    metadata = Session_Metadata(session_folder)
    Arduino_fpath = metadata.meta_dict['Arduino']
    behavior_fpath = metadata.meta_dict['BehaviorData']
    timestamp_fpath = metadata.meta_dict['timestamps']
    ...
    try:
        if v4:
            miniscope_file = [folder for folder in timestamp_fpath
                              if 'Miniscope' in os.path.split(folder)[0]][0]
            behavior_file = [folder for folder in timestamp_fpath
                              if 'BehavCam' in os.path.split(folder)[0]][0]
            sync_map, DAQ_data = sync_cameras_v4(miniscope_file,
                                                 behavior_file)
        else:
        ...
        # If not, sync Arduino data.
            ...
            if not self.paths['BehaviorData']:
                try:
                    ...
                    self.paths = Session_Metadata(self.folder,
                                                  overwrite=True).meta_dict
                    self.paths['PreprocessedBehavior'] = os.path.join(
                        self.folder, 'PreprocessedBehavior.csv')
            ...
            self.behavior_df = \
                sync_Arduino_outputs(self.folder,
                                     behav_cam=behav_cam,
                                     miniscope_cam=miniscope_cam,
...
class BehaviorSession:
    ...
    def get_learning_curve(self, trial_threshold=5):
        ...
        smoothed = savgol_filter(correct_responses,
                                 round_up_to_odd(self.ntrials/3), 2)
