...
class DREADDs:
    ...
    def plot_best_performance_all_sessions(
        ...
        df = pd.concat([pd.concat({k: pd.Series(v) for k,v in performance[session].items()}
                                  ) for session in sessions]
                       , axis=1, keys=sessions)
    ...
    def plot_behavior_grouped(self,
        ...
        df = pd.DataFrame(performance_all, index=np.hstack((self.meta['grouped_mice']['vehicle'],
                                                            self.meta['grouped_mice']['PSEM'])), columns=sessions)
